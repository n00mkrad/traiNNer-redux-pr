...
class PairedTemporalDataset(BaseDataset):
    """Paired temporal dataset that loads multi-frame GT for true temporal training.

    Unlike PairedVideoDataset which only loads the center GT frame,
    this dataset loads GT for ALL frames in each clip, enabling
    temporal losses and multi-frame output supervision.

    Expects 1:1 LQ and GT folders with matching filenames (same as
    PairedVideoDataset). The difference is purely in what gets loaded.

    Returns:
        lq: (clip_size, C, H, W) - all LQ frames in the clip
        gt: (clip_size, C, H, W) - all GT frames in the clip
    """
    ...
    def __init__(self, opt: DatasetOptions) -> None:
        ...
        self.clips: list[tuple[list[str], list[str]]] = []  # (lr_paths, hr_paths)
        ...
        for i, lq_path in enumerate(self.dataroot_lq):
            ...
            for start_idx in range(max(n_clips, 0)):
                ...
                # All frames in the clip must have GT
                if all(f in hr_files for f in scene_files):
                    lr_paths = [os.path.join(lq_path, f) for f in scene_files]
                    hr_paths = [os.path.join(gt_path, f) for f in scene_files]
                    self.clips.append((lr_paths, hr_paths))
        ...
        logger.info(
            "Found %d valid temporal clip pairs across %d scenes.",
            total_clips,
            total_scenes,
        )
    ...
    def __getitem__(self, idx: int) -> DataFeed:
        ...
        lr_paths, hr_paths = self.clips[idx]
        ...
        for i in range(self.clip_size):
            ...
            if self.opt.phase == "train":
                ...
                # Augment GT and LQ together with identical transforms
            ...
            img_lq = img2tensor(img_lq, from_bgr=False, float32=True)
            img_gt = img2tensor(img_gt, from_bgr=False, float32=True)

            lr_clip.append(img_lq)
            gt_clip.append(img_gt)

        return {
            "lq": torch.stack(lr_clip),
            "gt": torch.stack(gt_clip),
            "gt_path": hr_paths[middle_idx],
            "lq_path": lr_paths[middle_idx],
        }

    @property
    def label(self) -> str:
        return f"{self.clip_size}-frame temporal sequence pairs"
