...
# --- Coarse-to-Fine Optical Flow Network ---


def local_correlation(feat1: Tensor, feat2: Tensor, radius: int = 4) -> Tensor:
    """Compute local correlation between two feature maps.

    ONNX/TensorRT compatible implementation using unfold + matmul.

    Args:
        feat1: First feature map (B, C, H, W)
        feat2: Second feature map (B, C, H, W)
        radius: Search radius (window size = 2*radius + 1)

    Returns:
        Correlation volume (B, (2r+1)^2, H, W)
    """
    B, C, H, W = feat1.shape
    window = 2 * radius + 1

    # Normalize features for stable correlation
    feat1 = F.normalize(feat1, dim=1)
    feat2 = F.normalize(feat2, dim=1)

    # Pad feat2 for local search window
    feat2_pad = F.pad(feat2, [radius] * 4, mode="replicate")

    # Unfold feat2 to extract local patches: (B, C, H, W) -> (B, C*win*win, H*W)
    feat2_unfold = F.unfold(feat2_pad, kernel_size=window, padding=0)
    # Reshape to (B, C, win*win, H, W)
    feat2_unfold = feat2_unfold.view(B, C, window * window, H, W)

    # Compute correlation: dot product between feat1 and each position in window
    # feat1: (B, C, H, W) -> (B, C, 1, H, W)
    feat1 = feat1.unsqueeze(2)
    # Correlation: sum over C dimension -> (B, win*win, H, W)
    corr = (feat1 * feat2_unfold).sum(dim=1)

    return corr


def warp_features(feat: Tensor, flow: Tensor) -> Tensor:
    """Warp features according to flow using gather-based bilinear interpolation.

    Args:
        feat: Feature map (B, C, H, W)
        flow: Optical flow (B, 2, H, W) in pixel units at feat's resolution
    ...
    # Create max bounds as tensors for ONNX compatibility
    max_x = torch.tensor(W - 1, device=feat.device, dtype=feat.dtype)
    max_y = torch.tensor(H - 1, device=feat.device, dtype=feat.dtype)
    ...
    p00 = feat_flat.gather(-1, idx_00).view(B, C, H, W)
    p01 = feat_flat.gather(-1, idx_01).view(B, C, H, W)
    p10 = feat_flat.gather(-1, idx_10).view(B, C, H, W)
    p11 = feat_flat.gather(-1, idx_11).view(B, C, H, W)
...
class PyramidFlowNet(nn.Module):
    """Two-level coarse-to-fine optical flow estimation.

    Architecture:
    - Level 1 (1/8): Coarse flow prediction from concatenated frames
    - Level 2 (1/4): Residual refinement with local correlation

    Improvements over single-scale:
    - Better large motion handling from coarse level
    - Sharp motion boundaries from fine level refinement
    - Explicit correlation guides refinement network

    TensorRT, BF16, and ONNX compatible.
    """
    ...
    def __init__(self, in_ch: int = 3, base_ch: int = 32) -> None:
        ...
        self.enc1 = nn.Sequential(
            nn.Conv2d(in_ch, base_ch, 3, 2, 1),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(base_ch, base_ch, 3, 1, 1),
            nn.LeakyReLU(0.2, inplace=True),
        )

        # Stage 2: 1/4 resolution (used for refinement level)
        ...
        # Stage 3: 1/8 resolution (coarse level)
        ...
        # Coarse flow predictor at 1/8 (takes concatenated features)
        self.coarse_pred = nn.Sequential(
            nn.Conv2d(base_ch * 8, base_ch * 4, 3, 1, 1),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(base_ch * 4, base_ch * 2, 3, 1, 1),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(base_ch * 2, 2, 3, 1, 1),
        )

        # Refinement network at 1/4
        # Input: warped src features + tgt features + upsampled coarse flow + correlation
        corr_channels = 81  # (2*4+1)^2 = 81 for radius=4
        refine_in = (
            base_ch * 2 * 2 + 2 + corr_channels
        )  # src_feat + tgt_feat + flow + corr

        self.refine_net = nn.Sequential(
            nn.Conv2d(refine_in, base_ch * 2, 3, 1, 1),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(base_ch * 2, base_ch * 2, 3, 1, 1),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(base_ch * 2, base_ch, 3, 1, 1),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(base_ch, 2, 3, 1, 1),
        )

        # Initialize flow predictions to zero for stable training start
        nn.init.zeros_(self.coarse_pred[-1].weight)
        nn.init.zeros_(self.coarse_pred[-1].bias)
        nn.init.zeros_(self.refine_net[-1].weight)
        nn.init.zeros_(self.refine_net[-1].bias)

    def forward(self, src: Tensor, tgt: Tensor) -> Tensor:
        """Estimate optical flow from src to tgt.

        Args:
            src: Source frame (B, C, H, W)
            tgt: Target/reference frame (B, C, H, W)
        ...
        # Encode both frames through shared encoder
        src_e1 = self.enc1(src)  # 1/2
        tgt_e1 = self.enc1(tgt)

        src_e2 = self.enc2(src_e1)  # 1/4
        tgt_e2 = self.enc2(tgt_e1)

        src_e3 = self.enc3(src_e2)  # 1/8
        tgt_e3 = self.enc3(tgt_e2)

        # === Coarse level (1/8) ===
        # Concatenate features and predict coarse flow
        coarse_cat = torch.cat([src_e3, tgt_e3], dim=1)
        flow_coarse = self.coarse_pred(coarse_cat)  # (B, 2, H/8, W/8)

        # Upsample coarse flow to 1/4 resolution (scale displacement by 2)
        H4, W4 = H // 4, W // 4
        flow_coarse_up = (
            F.interpolate(
                flow_coarse, size=(H4, W4), mode="bilinear", align_corners=False
            )
            * 2.0
        )

        # === Fine level (1/4) ===
        # Warp source features using upsampled coarse flow
        src_e2_warped = warp_features(src_e2, flow_coarse_up)

        # Compute local correlation between warped source and target
        corr = local_correlation(src_e2_warped, tgt_e2, radius=4)

        # Concatenate for refinement: warped_src + tgt + flow + correlation
        refine_input = torch.cat([src_e2_warped, tgt_e2, flow_coarse_up, corr], dim=1)
        flow_residual = self.refine_net(refine_input)

        # Combined flow at 1/4 resolution
        flow_fine = flow_coarse_up + flow_residual

        # Upsample to full resolution (scale by 4 for pixel displacement)
        flow = (
            F.interpolate(flow_fine, size=(H, W), mode="bilinear", align_corners=False)
            * 4.0
        )
